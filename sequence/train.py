...
model = EncoderNet(d_model=1280, num_heads=4, num_layers=1, d_ff=2560, dropout_l=0.1, dropout=0.25,
...
def update_metrics(mode, epoch, running_loss, true_positive_live, false_positive_live, true_negative_live,
    ...
    if prob < metrics[mode]['AE']:
        ...
        if mode == 'test':
            torch.save(model, f"./weights/{cfg.backbone_name}@{crop_format}_encoder.pth")
